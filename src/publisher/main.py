# Publisher UI v0.4 (visual friendly)
import argparse
import asyncio
import contextlib
import json
...
def status_html(text: str, color: str) -> str:
    return f"<span style='color:{color}; font-size:1em; vertical-align:middle; line-height:1;'>■</span> {text}"
...
class PublisherUI(QWidget):
    def __init__(self, backend_ws_url: str, pin: str) -> None:
        ...
        self.setWindowTitle("BYOD Publisher UI v0.4")
        self.resize(600, 560)
    ...
    def _build_layout(self) -> None:
        self.setStyleSheet(
            ...
            QComboBox QAbstractItemView {{ background-color: #3A3A3A; color: #D6D6D6; border: 2px solid #555; }}
            QComboBox QAbstractItemView::item:selected {{ background-color: #8A8A8A; color: #1E1E1E; }}
            """
        )
    ...
    def _ui_set_button_state(self, channel_id: str, text: str, enabled: bool, mode: str) -> None:
        ...
        if mode == "streaming":
            ...
            btn.setStyleSheet("background-color: #3E5276; color: #E8E8E8; border:1px solid #33445F; border-radius:4px; padding:4px 10px;")
        else:
            btn.setStyleSheet("")

...
## 9. Импорт room config из /tmp

Backend port `8000` остаётся закрытым снаружи, а `/admin/*` остаётся local-only и не публикуется через nginx. SwaggerUI на `:8000/docs` не является публичным operator workflow для VPS. Используйте локальный file workflow на самом VPS.

После загрузки файла на VPS выполните:
...
Скрипт проверяет JSON syntax, делает backup текущего `/opt/byod/backend_data/room_config_v1.json`, атомарно заменяет файл, ставит owner `byod:byod`, restart backend и проверяет локальный `/health`. Если import fails, не удаляйте backup; соберите diagnostics и вывод ошибки.

## 10. Установка LiveKit browser client vendor из /tmp

Если pinned файл `livekit-client.umd.1.15.13.js` был загружен на VPS отдельно, установите его локально без изменения vendor workflow в репозитории:

...
func Classify(target int, c Counts) string {
	...
	mediaOK := !c.MediaRequired || (c.AudioTargetReached && c.RTPTargetReached && c.RTPPackets > 0 && c.RTPBytes > 0)
	livekitHoldOK := !c.LiveKitRequired || c.LiveKitDisconnectedDuringHold == 0
	terminalOK := c.WorkersWithoutTerminalEvent == 0
	if target > 0 && c.Started >= target && c.BackendTargetReached && livekitOK && mediaOK && c.HoldCompleted && c.BackendRejected == 0 && c.BackendClosedDuringHold == 0 && c.HeartbeatFailedDuringHold == 0 && c.RTPReadErrors == 0 && livekitHoldOK && terminalOK && !c.FatalSetupError {
		return "VALID_RUN"
	}
	if c.Started > 0 || c.BackendConnected > 0 || c.BackendActive > 0 || c.BackendRejected > 0 || c.BackendClosed > 0 || c.Errors > 0 || c.LiveKitConnected > 0 || c.LiveKitFailed > 0 || c.LiveKitDisconnected > 0 || c.AudioTracksSubscribed > 0 || c.WorkersWithRTP > 0 || c.RTPPackets > 0 || c.RTPReadErrors > 0 {
		return "PARTIAL_RUN"
	}
	return "INVALID_RUN"
}

...
func TestClassify(t *testing.T) {
	if got := Classify(2, Counts{Started: 2, BackendConnected: 2, BackendActive: 2, ActiveTargetReached: true, HoldCompleted: true}); got != "VALID_RUN" {
		t.Fatal(got)
	}
...
func TestClassifyRejectsHoldFailures(t *testing.T) {
	base := Counts{Started: 2, BackendConnected: 2, BackendActive: 2, ActiveTargetReached: true, HoldCompleted: true}

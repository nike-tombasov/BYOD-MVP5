	...
	go func() {
		...
		for {
			...
			if err := c.ReadJSON(&msg); err != nil {
				if ctx.Err() == nil {
					if mode == "livekit-connect-only" && !gotListenerState {
						e := withKind(eventBase, "error")
			...
			if msg["type"] == "listener_state" {
				...
				if mode == "livekit-connect-only" && !liveKitConnected {
					if info.Token == "" {
						e := withKind(eventBase, "error")
						e.ListenerID = listenerID
					...
					e.LiveKitURL = info.URL
					events <- e
					room, err := connector.Connect(ctx, info.URL, info.Token)
					if err != nil {

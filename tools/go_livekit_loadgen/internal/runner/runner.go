...
func (r *Runner) Run(ctx context.Context) error {
	...
	r.counts.LiveKitRequired = r.C.Mode == config.ModeLiveKitConnectOnly
	...
	for !holdCompleted {
		...
		case e := <-events:
			...
			if !holdStarted && r.shouldStartHold() {
				...
				if r.C.Mode == config.ModeLiveKitConnectOnly {
...
func (r *Runner) shouldStartHold() bool {
	...
	livekitReady := r.C.Mode != config.ModeLiveKitConnectOnly || r.counts.LiveKitActive >= r.C.Listeners
	return backendReady && livekitReady && !r.counts.FatalSetupError && r.counts.BackendRejected == 0 && r.counts.BackendClosed == 0 && r.counts.LiveKitFailed == 0
}
...
func (r *Runner) targetCannotBeReached() bool {
	...
	return r.counts.RampDone && baseFailure && (r.counts.BackendActive < r.C.Listeners || (r.C.Mode == config.ModeLiveKitConnectOnly && r.counts.LiveKitActive < r.C.Listeners))
}
...
func (r *Runner) liveLine(holdStart time.Time, holdStarted bool) string {
	return fmt.Sprintf("ts_iso=%s mode=%s profile=%s target_listeners=%d target_ws=%s started=%d backend_active=%d backend_rejected=%d backend_closed=%d livekit_connected=%d livekit_failed=%d livekit_disconnected=%d transport_udp=%d transport_tcp=%d transport_unknown=%d udp_tcp_ratio=%s heartbeat_ok=%d heartbeat_failed=%d ramp_done=%t hold_elapsed=%.0fs errors_top=%v", TS(), r.C.Mode, r.C.Profile, r.C.Listeners, r.target, r.counts.Started, r.counts.BackendActive, r.counts.BackendRejected, r.counts.BackendClosed, r.counts.LiveKitConnected, r.counts.LiveKitFailed, r.counts.LiveKitDisconnected, r.counts.TransportUDP, r.counts.TransportTCP, r.counts.TransportUnknown, r.udpTCPRatio(), r.counts.HeartbeatOK, r.counts.HeartbeatFailed, r.counts.RampDone, holdElapsedSeconds(holdStart, holdStarted), r.topErrors())
}

func (r *Runner) eventPayload(e backendws.Event) map[string]any {
	return map[string]any{"ts_iso": TS(), "event": e.Kind, "runner_id": r.C.RunnerID, "worker_id": e.WorkerID, "worker_index": e.WorkerIndex, "mode": r.C.Mode, "profile": r.C.Profile, "listener_id": e.ListenerID, "livekit_url": e.LiveKitURL, "transport": e.Transport, "error": e.Error, "close_code": e.CloseCode}
}
